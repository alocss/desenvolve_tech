...
**Segurança e operação** — rate limit global via `ThrottlerModule` no backend (30 req/min por padrão; endpoints sensíveis usam limite mais restrito via `@Throttle`, ex.: `/contact` a 5/hora). Revisão de segurança obrigatória em qualquer PR que toque autenticação, dados pessoais ou endpoints públicos. Backend e frontend já fisicamente separados (dois projetos, dois deploys). Termos de uso/política de privacidade: pendente — necessário antes do lançamento público, dado que o formulário de contato (Issue #7) coleta dados pessoais.

**Performance budget e SEO** — Lighthouse CI (`lighthouserc.json` na raiz, job `lighthouse` do CI) roda contra as 5 páginas do site via `next start` local, com limiares: performance ≥ 0,75, acessibilidade ≥ 0,95, SEO ≥ 0,95, boas práticas ≥ 0,90. **Atenção ao reavaliar após o deploy (Issue #9)**: esses números foram calibrados testando contra `next start` num processo Node local, não contra o hosting/CDN real de produção — o comportamento pode (e deve) melhorar depois do deploy; revisitar os limiares nesse momento em vez de assumir que são o teto. Regra aprendida nesta issue: nunca envolver o conteúdo acima da dobra (hero) em animação de entrada via Framer Motion (`Reveal`/`RevealGroup`) — como o elemento nasce com `opacity: 0` até a hidratação do React rodar, isso atrasa o LCP em vários segundos. Hero usa a classe utilitária `.hero-enter` (CSS puro, `globals.css`) em vez disso; `Reveal`/`RevealGroup` continuam certos para conteúdo abaixo da dobra, onde a métrica de LCP não é afetada.
